...
락 없이 풀 수 있었던 이유는 **판정과 쓰기가 SQL 한 문장에 들어갔기 때문**이다. 그 사이에 끼어들 틈이 없으니 잠글 이유가 없다.

```sql
UPDATE ... WHERE id=? AND balance >= :amount         -- 잔액 판정 + 차감
UPDATE ... WHERE no=? AND status not in (terminal)   -- 상태 판정 + 전이
INSERT ... (uk_merchant_payment_no)                   -- 존재 판정 + 생성
```

**운이 좋았던 게 아니라 도메인이 단순해서 가능했다.** 네 가지 전제가 성립한다.

| 전제 | 현재 | 깨지면 |
|---|---|---|
| **단일 행** | 지갑 1개, 결제 1건 | 지갑 간 이체 → 두 행을 한 스냅샷으로 봐야 한다 |
| **고정 금액** | 요청에 금액이 담겨 온다 | "잔액의 N% 수수료" → 읽은 값 기준 계산이 필요하다 |
| **단방향 상태 머신** | `PENDING → 종료`, 되돌아가지 않는다 | 부분 취소·환불 → 종료 상태가 더 이상 종료가 아니다 |
| **불변식이 한 행 안에** | 잔액 ≥ 0 | 일일 한도 합계 → 여러 행 집계라 행 락으로 못 막는다(팬텀) |

**하나라도 깨지면 락을 다시 검토해야 한다.** 구체적으로:

- **부분 취소가 생기면** `COMPLETED → PARTIALLY_REFUNDED` 전이가 필요해져 `not in (COMPLETED, FAILED)` 조건이 그대로는 성립하지 않는다.
- **지갑 간 이체가 생기면** 차감과 증가를 한 스냅샷으로 봐야 한다. 조건부 UPDATE 두 번으로는 중간 상태가 노출되고, 락 순서가 꼬이면 데드락이다.
- **일일 한도가 생기면** 새 행 INSERT를 행 락이 막지 못하므로 별도 카운터 행이나 상위 격리 수준이 필요하다.

### 남아 있는 read-then-act

이 결정으로 결제 경로는 닫혔지만 **충전 경로에는 틈이 남아 있다.**

```java
findByChargeNo(...)      // ① 조회
chargeStore.append(...)  // ② INSERT — UNIQUE가 막는다
walletStore.charge(...)  // ③ UPDATE ... WHERE id=? — 조건 없음
```

③에는 조건이 없다. 이중 충전이 발생하지 않는 것은 **②의 UNIQUE가 앞에서 막기 때문**이지 ③이 안전해서가 아니다. 즉 충전의 멱등성은 원장 테이블(`wallet_charge`)에 의존한다.

같은 이유로 **차감에는 원장이 없다는 비대칭**이 근본 문제로 남는다([SPEC §11](../SPEC.md) 항목 E). CAS는 그 비대칭을 메운 것이 아니라 우회한 것이다.

현재 정합성을 떠받치는 것은 **UNIQUE 2개 + 조건부 UPDATE 2개 + CHECK 3개**, 총 7개다. 락은 0건이다.
